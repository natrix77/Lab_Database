...
from PyQt5.QtWidgets import (
    ...
    QTableWidgetItem, QHBoxLayout, QComboBox, QFileDialog
)
...
class AssignTeamsTab(QWidget):
    ...
    def init_ui(self):
        ...
        self.export_button = QPushButton("Export Teams to Excel")
        self.export_button.clicked.connect(self.export_teams_to_excel)
    ...
    def export_teams_to_excel(self):
        # Use the same semester/year selection logic as in the show_teams function
        semester, year = self.select_or_add_semester_year()
        if not semester or not year:
            return  # Exit if the user canceled the selection
        ...
        result = cursor.fetchone()
        if not result:
            QMessageBox.warning(self, "Warning", "No data found for the selected semester and year.")
            conn.close()
            return
        academic_year_id = result[0]
        ...
        conn.close()
        if not lab_slots:
            QMessageBox.warning(self, "Warning", "No lab slots found for the selected academic year.")
            return

        # Let user select a lab slot
        lab_slot_names = [slot[1] for slot in lab_slots]
        lab_slot_name, ok = QInputDialog.getItem(self, "Select Lab Slot", "Lab Slot:", lab_slot_names, 0, False)
        if not ok:
            return
        ...
        # Export teams for this lab slot
        self.export_teams_for_lab_slot(lab_slot_id, lab_slot_name)

    def export_teams_for_lab_slot(self, lab_slot_id, lab_slot_name):
        conn = sqlite3.connect('student_register.db')
        cursor = conn.cursor()
        # Fetch the teams for this lab slot
        cursor.execute('''
            SELECT s.student_id, s.name, st.team_number
            FROM Students s
            INNER JOIN StudentTeams st ON s.student_id = st.student_id
            WHERE st.lab_slot_id=?
        ...
        if not teams:
            QMessageBox.information(self, "Info", "No teams found for the selected lab slot.")
            return

        # Convert the data into a DataFrame for exporting to Excel
        df = pd.DataFrame(teams, columns=["Student ID", "Student Name", "Team Number"])

        # Sanitize lab slot name to remove any invalid filename characters
        sanitized_lab_slot_name = re.sub(r'[\\/*?:"<>|]', "", lab_slot_name)

        # Format filename with sanitized lab slot name
        filename = f"Teams_{sanitized_lab_slot_name}.xlsx"

        # Ask the user where to save the Excel file
        file_path, _ = QFileDialog.getSaveFileName(self, "Save Excel File", filename, "Excel Files (*.xlsx)")
        if file_path:
            try:
                df.to_excel(file_path, index=False)
                QMessageBox.information(self, "Success", f"Teams exported successfully to {file_path}.")
            except Exception as e:
                QMessageBox.critical(self, "Error", f"Failed to export teams: {e}")
